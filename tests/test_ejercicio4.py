...
class TestAnalyzeDataset(unittest.TestCase):
    ...
    def test_genera_estadisticas_globales(self, mock_file, mock_makedirs):
        ...
        handle = mock_file()
        written_data = ''.join(call.args[0] for call in handle
        ...
        self.assertIsInstance(globales['correlacion_abandono_rendimiento'],
                              float)
    ...
    def test_guarda_archivo_json(self, mock_file, mock_makedirs):
        ...
        # Verificar que se llamó makedirs (independiente del separador de path)
        mock_makedirs.assert_called_once()
        call_args = mock_makedirs.call_args[0][0]
        self.assertIn('report', call_args)

        # Verificar que se abrió el archivo (independiente del separador)
        mock_file.assert_called_once()
        file_path = mock_file.call_args[0][0]
        self.assertIn('analisi_estadistic.json', file_path)
